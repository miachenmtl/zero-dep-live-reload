      ...
      if (isConnected) {
        ...
        */
        ...
        connection.write(message, () => {
          console.log(
            `[${new Date().toLocaleTimeString()}]: Reload message sent to client.`
          );
        });
      } else {
        console.log(
          `[${new Date().toLocaleTimeString()}]: ${WATCHED_FILENAME} updated, but nothing is connected!`
        );
      }
    }
  }
...
server.on("upgrade", (req, socket) => {
  ...
  socket.on("data", (data) => {
    if (data[0] === 0x8a && data[1] === 0x84) {
      // pong received
      if (data.length !== 10)
        console.error("Incorrect pong payload length received!");
      else {
        const mask = data.subarray(2, 6);
        const payload = data.subarray(6, 10);
        const unmaskedText = Buffer.from(
      ...
      console.log(`Unexpected data received: ${data.toString()}`);
    }
  });
  socket.on("timeout", () => {
  ...
  socket.on("close", (hadError) => {
    ...
    console.log(
      hadError
        ? "Connection closed due to transmission error!"
        : "Connection closed."
    );
    console.log("=========================");
  });
...
server.listen(PORT, () => {
  console.log(
    `[${new Date().toLocaleTimeString()}]: Server ready on localhost:${PORT}.`
  );
  console.log("=========================");
});
